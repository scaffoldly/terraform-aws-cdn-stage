locals {
  ...
  aliases                = length(var.cdn_domains) > 0 ? concat([local.domain], formatlist("%s.${var.root_domain}", formatlist("%s%s", var.additional_subdomains, var.subdomain_suffix != "" ? "-${var.subdomain_suffix}" : ))) : null
}
